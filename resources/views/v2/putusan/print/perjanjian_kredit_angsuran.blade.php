	...
	<body>
		<div class="container" style="width: 21cm;height: 29.7cm;font-size:11px">
			...
			<div class="row text-justify">
				<div class="col-xs-12">
					<p>
						Pada hari ini {{ucwords($hari[strtolower(Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->format('l'))])}} tanggal {{Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->format('d/m/Y')}}, telah dibuat dan ditandatangani perjanjian antara :
					</p>
					...
					<p>
						<ol>
							...
							<li>
								Pengembalian pinjaman atau kredit wajib dilakukan oleh PEMINJAM dengan cara mengangsur bunga dan/atau pokok pinjaman tiap bulan  dan dilakukan secara berturut-turut tanpa adanya suatu tunggakan atau penangguhan dengan jumlah angsuran per-bulan sebesar {{$angsuran['angsuran'][1]['total_angsuran']}} ( {{\App\Http\Service\UI\Terbilang::dariRupiah($angsuran['angsuran'][1]['total_angsuran'])}}), dengan jangka waktu pinjaman {{$data['putusan']['jangka_waktu']}} ( {{\App\Http\Service\UI\Terbilang::terbilang($data['putusan']['jangka_waktu'])}}) bulan sejak tanggal PERJANJIAN ini ditandatangani, dibayar dalam  {{$data['putusan']['jangka_waktu']}} ( {{\App\Http\Service\UI\Terbilang::terbilang($data['putusan']['jangka_waktu'])}}) kali angsuran, pada tanggal {{Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->format('d')}} ( {{\App\Http\Service\UI\Terbilang::terbilang(Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->format('d'))}} ) setiap bulannya, pengembalian pinjaman atau kredit berlaku mulai {{Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->addmonthsnooverflow(1)->format('d/m/Y')}} dan berakhir pada {{Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->addmonthsnooverflow($data['putusan']['jangka_waktu'])->format('d/m/Y')}}, hingga seluruh pinjaman baik berupa pokok, bunga maupun biaya-biaya lainnya telah lunas;
							</li>
							<li>
				...
				<div class="col-xs-12">
					<p>
						<ol>
							...
							<li>
								Seluruh hutang PEMINJAM berikut denda dan biaya-biaya lain yang timbul kepada {{strtoupper($pimpinan['kantor']['jenis'])}} dapat ditagih secara seketika dan sekaligus bilamana :
								<ol type="a">
									<li>
										PEMINJAM meninggal dunia kecuali jika para ahli waris dan atau yang memperoleh hak dapat memenuhi seluruh kewajiban PEMINJAM dan memperoleh persetujuan {{strtoupper($pimpinan['kantor']['jenis'])}};
									</li>
									<li>
									...
									<li>
										Barang agunan tersebut dipindah tangankan atau dijaminkan kepada pihak ketiga  tanpa mendapatkan persetujuan tertulis dari {{strtoupper($pimpinan['kantor']['jenis'])}};
									</li>
									<li>
							...
							<li>
								Bilamana PEMINJAM bermaksud melunasi hutangnya kepada {{strtoupper($pimpinan['kantor']['jenis'])}} sebelum berakhirnya masa angsuran atau disebut dengan pelunasan dipercepat, maka PEMINJAM menyetujui untuk :
								<ol type="a">
									<li>
					...
					<p>
						Demikian syarat-syarat tersebut diatas merupakan satu kesatuan yang tidak terpisahkan dari SURAT PERJANJIAN KREDIT. No. {{$putusan['nomor_kredit']}} tertanggal {{Carbon\Carbon::createfromformat('d/m/Y H:i', $data['pengajuan']['status_realisasi']['tanggal'])->format('d/m/Y')}} yang telah dimengerti dan disepakati oleh kedua belah pihak.
					</p>
				</div> 
			</div> 
