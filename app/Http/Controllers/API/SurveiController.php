...
class SurveiController extends BaseController
{
	...
	public function index()
	{
		try {
			//1. find pengajuan
			if(request()->has('nip_karyawan')){
				$pengajuan	= Survei::wherehas('pengajuan', function($q){$q->status('survei');})->where('surveyor->nip', request()->get('nip_karyawan'))->wherehas('collateral', function($q){$q->wheredoesnthave('foto', function($q){$q;});})->with(['collateral', 'collateral.foto'])->get();
			}
			else{
